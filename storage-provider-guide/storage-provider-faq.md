...
2. Download new version (e.g., `es-node.v0.2.2`) of the pre-built package suitable for your platform using commands [here](/storage-provider-guide/tutorials.md#from-pre-built-executables).

3. Some files including data folder need to be moved from the directory of old build (e.g., `es-node.v0.2.1`) to the new one:

```sh
# replace v0.2.2 to your target version
cd es-node.v0.2.2

# replace v0.2.1 to the version you have
mv ../es-node.v0.2.1/es-data .
mv ../es-node.v0.2.1/esnode_* .
```
...
2. Switch to the correct branch of the source code (e.g., you want to move to `v0.2.2`):
